...
function newMaterialFormSubmit(event, form) {
    event.preventDefault();
    //get data
    var item = form.description.value;
    var brand = form.brand.value;
    var cost = form.currentCost.value;
    //save data 
    //if successful, add row and close modal
    ...
    td[0].innerHTML = "<input type='text' class='form-control-sm' name='description' value='" + item + "' readonly disabled='disabled'></input>";
    td[1].innerHTML = '<input type="text" class="form-control-sm" name="brand" value="' + brand + '" readonly disabled="disabled">';
    td[2].innerHTML = '<input type="text" class="form-control-sm" name="currentCost" value="$' + cost +  '" readonly disabled="disabled">';
    td[3].textContent = "0";
    td[4].textContent = "$0.00";
    td[5].innerHTML = '<button class="editButton" data-open="0"><svg class="editIcon" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-fill" viewBox="0 0 16 16"><path d="M12.854.146a.5.5 0 0 0-.707 0L10.5 1.793 14.207 5.5l1.647-1.646a.5.5 0 0 0 0-.708zm.646 6.061L9.793 2.5 3.293 9H3.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.207zm-7.468 7.468A.5.5 0 0 1 6 13.5V13h-.5a.5.5 0 0 1-.5-.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.5-.5V10h-.5a.5.5 0 0 1-.175-.032l-.179.178a.5.5 0 0 0-.11.168l-2 5a.5.5 0 0 0 .65.65l5-2a.5.5 0 0 0 .168-.11z" /></svg></button>';
    //don't move >:(
    ...
    //         // Need to get the marerial id from the response for later
    //         var materialId = data.blahblahblah;
    //         clone.dataset.materialId = materialId;
               form.reset();
    //     },
    //     error: function(data) {
    ...
    // If data was stored successfully, then close the dialog
    $('#newMaterialModal').modal('hide')
};
...
function materialUpdateHandler(event) {
    ...
    var url = "/user/{userId}/materials?materialId=" + materialId;
    // do the ajax put
    // $.ajax({
    //     type: "PUT",
    //     url: url,
